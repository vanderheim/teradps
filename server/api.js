            ...
            var personalBest = PersonalRankings.findOne({
              ...
              mainPlayer: this.bodyParams.members[z].playerName
						...
						for (var s = 0; s < this.bodyParams.members[z].buffUptime.length; s++) {
							if (this.bodyParams.members[z].buffUptime[s].Key == 8888889) {
								if (parseInt(this.bodyParams.members[z].buffUptime[s].Value) > 20) {
									solo.slaying = "Y";
								} else {
									solo.slaying = "N";
								}
                ...
                continue;
						...
						if (bestFreebie !== undefined && bestFreebie.length === 5 && parseInt(this.bodyParams.members[z].playerDps) > bestFreebie[0].mainDps) {
							var defeated = false;
